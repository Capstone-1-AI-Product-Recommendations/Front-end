...
import productData from "../../data/product"; // Import sản phẩm từ file dữ liệu
...
const FeaturedProduct = () => {
  ...
  useEffect(() => {
    let interval;
    if (hoveredProduct !== null && productData[hoveredProduct].altImages) {
      interval = setInterval(() => {
        setCurrentImageIndex((prev) => ({
          ...prev,
          [hoveredProduct]:
            (prev[hoveredProduct] || 0) + 1 >= productData[hoveredProduct].altImages.length
              ? 0
